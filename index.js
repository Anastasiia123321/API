const buttonGetPersons = document.getElementById('get-actors');
...
buttonGetPersons.addEventListener('click', async function () {
    ...
    const res = await axios.get(BASE + 'films/' + filmIndex + '/');
...
async function GetPlanets(page) {
    const planetsOfFilms = await axios.get(BASE + 'planets/?page=' + page);
    continerDiv.innerHTML = '';
    if (currentPage == Math.ceil(planetsOfFilms.data.count / numberPlanetsOnPage)) {
        nextBtn.classList.add('hidden-btn');
        nextBtn.classList.remove('visible-btn');
    } else if (currentPage == Math.ceil(planetsOfFilms.data.count / numberPlanetsOnPage) - 1) {
        nextBtn.classList.remove('hidden-btn');
        nextBtn.classList.add('visible-btn');
    }
